...
function getETA(ph,pm,ps,id){ //MadnessRed function
	//new date
	var eta=new Date();

	//Add our values to it.
	var h=(eta.getHours() + (ph * 1));
	var m=(eta.getMinutes() + (pm * 1));
	var s=(eta.getSeconds() + (ps * 1));
	...
	t=setTimeout('getETA('+ph+','+pm+','+ps+',\''+id+'\')',500);
}
...
function shortInfo() {
	...
	//MadnessRed addition, start the getETA function, much easier to do it here than anywhere else.
	getETA((hours*1),(minutes*1),(seconds*1),'arrivalTime');
	getETA((hours*2),(minutes*2),(seconds*2),'returnTime');
	//End of MadnessRed stuff
