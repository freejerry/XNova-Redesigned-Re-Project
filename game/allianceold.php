 ...
 * WYSIWYG Editor eingefügt von Mwieners
 */
...
if ($_GET['mode'] == 'ainfo') {
	...
	// query
	//lang alliance info moved to the lang files :)
	...
	if (isset($_GET['tag'])) {
		...
		message($lang['Alliance_no_exist'], $lang['Alliance_information']);
	}
	// Si no existe
	if (!$allyrow) {
		message($lang['Alliance_no_exist'], $lang['Alliance_information']);
	...
	if ($ally_description != "") {
		...
		$ally_description = "<tr><th colspan=2 height=100>".$lang['Alliance_no_desc']."</th></tr>";
	...
	$patterns[] = '#\[h1\](.*?)\[/h1\]#Smi';
	$replacements[] = '<font size=16>\1</font>';
	$patterns[] = "#\[fc\]([a-z0-9\#\ \[\]]+)\[/fc\]#Ssi";
	$replacements[] = '<font color="\1">';
	$patterns[] = "#\[/f\]#Ssi";
	$replacements[] = '</font>';
	$patterns[] = '#\[youtube\](.*?)\[/youtube\]#Ssi';
	$replacements[] = '<object width="425" height="344"><embed src="http://www.youtube.com/v/\1" type="application/x-shockwave-flash" allowfullscreen="false" width="425" height="344"></embed></object>';

	$ally_description = preg_replace($pattern, $replace, $ally_description);
	...
	if ($user['ally_id'] == 0) {
		$lang['bewerbung'] = "<tr>
	  <th>Bewerben</th>
	  <th><a href=\"alliance.php?mode=apply&amp;allyid=" . $id . "\">".$lang['write_apply']."</a></th>
...
if ($user['ally_id'] == 0) { // Sin alianza
	...
	*/
		...
		*/
			...
			header("Location: ./?page=network&s=".UNI);
		} else {
			displaypage(parsetemplate(gettemplate('network/noalliance'), $lang), $lang['alliance']);
		}
	...
	*/
	...
	if ($mode == 'apply' && $user['ally_request'] == 0) { // solicitudes
		...
		if ($_POST['further'] == $lang['Send']) { // esta parte es igual que el buscador de search.php...
			doquery("UPDATE {{table}} SET `ally_request`='" . intval($allyid) . "', ally_request_text='" . strip_tags ( $_POST['text'], '<br><a><i><p><u><table><td><tr><em><h1><h2><h3><h4><li>ul><ol><blockquote><img><span><font><tbody><sub><sup><flash><windowsmedia><object>' ) . "', ally_register_time='" . time() . "' WHERE `id`='" . $user['id'] . "'", "users");
...
*/
	...
	*/
		...
		if ($sendmail == 1) {
			...
			$_POST['text'] = strip_tags ( $_POST['text'], '<br><a><i><p><u><table><td><tr><em><h1><h2><h3><h4><li>ul><ol><blockquote><img><span><font><tbody><sub><sup><flash><windowsmedia><object>' );
	...
	if ($mode == 'admin' && $edit == 'ally') { // Administrar la alianza *pendiente urgente*
		...
		if ($_POST) {
			if (!get_magic_quotes_gpc()) {
				...
				$_POST['text'] = stripslashes($_POST['text'], '<br><a><i><p><u><table><td><tr><em><h1><h2><h3><h4><li>ul><ol><blockquote><img><span><font><tbody><sub><sup><flash><windowsmedia><object>' );
			}
		}
		...
		if ($_POST['options']) {
			...
			if ($t == 3) {
				$ally['ally_request'] = strip_tags ( $_POST['text'], '<br><a><i><p><u><table><td><tr><em><h1><h2><h3><h4><li>ul><ol><blockquote><img><span><font><tbody><sub><sup><flash><windowsmedia><object>' );

				doquery("UPDATE {{table}} SET
				`ally_request`='{$ally['ally_request']}'
				WHERE `id`='{$ally['id']}'", "alliance");
			} elseif ($t == 2) {
				$ally['ally_text'] = strip_tags ( $_POST['text'], '<br><a><i><p><u><table><td><tr><em><h1><h2><h3><h4><li>ul><ol><blockquote><img><span><font><tbody><sub><sup><flash><windowsmedia><object>' );
				doquery("UPDATE {{table}} SET
				`ally_text`='{$ally['ally_text']}'
				WHERE `id`='{$ally['id']}'", "alliance");
			} else {
				$ally['ally_description'] = strip_tags ( $_POST['text'], '<br><a><i><p><u><table><td><tr><em><h1><h2><h3><h4><li>ul><ol><blockquote><img><span><font><tbody><sub><sup><flash><windowsmedia><object>' );
	...
	*/
		...
		$lang['Disolve_alliance'] = MessageForm($lang['Alliance_dissolve'], "", "?mode=admin&edit=exit", $lang['Continue']);
	...
	*/
	...
	if ($mode == 'admin' && $edit == 'requests') { // Administrar solicitudes
		...
		if ($_POST['action'] == "Akzeptieren") {
			...
			`message_text`='".str_replace("{-{alliance}-}",$ally['ally_name'],$lang['App_yesplease']) . $_POST['text'] ."'", "messages");
			...
			`message_text`='".str_replace("{-{alliance}-}",$ally['ally_name'],$lang['App_nothanks']) . $_POST['text'] ."'", "messages");
		...
		displaypage($page, $lang['Check_the_requests']);
	}
	...
	if ($mode == 'admin' && $edit == 'name') {
		...
		displaypage($page, $lang['Alliance_admin']);
	...
	*/
	...
	if ($mode == 'admin' && $edit == 'transfer') {
		...
		if ($ally['ally_owner'] != $user['id']) {
			...
			displaypage($page, $lang['Members_list']);
		}
	...
	// Default *falta revisar...*
		...
		displaypage($page, $lang['alliance']);
	}
}
